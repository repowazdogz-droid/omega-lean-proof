...
> **Superseded (2026-09-25 note; the sentence above is preserved unchanged).** The clause "a single declared user axiom (`compute_hash_collision_resistant`)" does not describe the shipped tree at or after commit `b5070b8` (2026-06-09), which removed that declaration; the shipped roots declare no user axiom. See the *Verification status* and *Correction* notes in the README. The README's *Assumptions* section states what the Lean development does and does not establish about the hash.
