...
theorem parseStringCharsGoFuel_mono_inner (m n : Nat) (s acc : List Char) (h : m ≤ n) :
    parseStringCharsGoFuel m s acc = parseStringCharsGoFuel n s acc := by
  induction h with
  | refl => rfl
  | step h ih =>
...
theorem parseStringCharsGoFuel_mono (m n : Nat) (s acc : List Char) (h : m ≤ n) :
  ...
  unfold parseStringCharsGoFuel
  exact parseStringCharsGoFuel_mono_inner m n s acc h
